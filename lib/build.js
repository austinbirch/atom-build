...
module.exports = {
  ...
  activate: function(state) {
    ...
    if (fs.existsSync(root + '/.atom-build.json')) {
      var atomBuild = fs.realpathSync(root + '/.atom-build.json');
      var realAtomBuild = require(atomBuild);
      if (_.isArray(realAtomBuild)) {
        _.each(realAtomBuild, function(build) {
          if (_.include(reservedCommands, build.name)) {
            return;
          }
          atom.commands.add(
            'atom-workspace',
            'build:' + build.name,
            self.build.bind(self, null, build.name)
          );
        });
      }
    }
  ...
  buildCommand: function(name) {
    ...
    if (fs.existsSync(root + '/.atom-build.json')) {
      ...
      if (_.isArray(build)) {
        build = name ? _.findWhere(build, {name: name}) : _.first(build);
      }
