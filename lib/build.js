...
var NamedBuildsView = require('./named-builds-view');
var BuildView = require('./build-view');
var tools = require('./tools');
var atomBuildTool = require('./tools/atom-build');
...
module.exports = {
  ...
  activate: function(state) {
    ...
    atom.commands.add('atom-workspace', 'build:refresh-build-config', function() {
      self.refreshBuildConfig();
      atom.commands.dispatch(workspaceElement, 'build:show-named-builds');
    });
    atom.commands.add('atom-workspace', 'build:show-named-builds', this.showNamedBuilds.bind(this));
  ...
  refreshBuildConfig: function() {
    var path = atom.project.getPaths()[0]; // Only support root path for now.
    if (atomBuildTool.isEligable(path)) {
      this.setupNamedBuilds();
    }
  },

  setupNamedBuilds: function() {
    var self = this;
    var path = atom.project.getPaths()[0]; // Only support root path for now.
    var namedBuilds = atomBuildTool.settings(path).builds;
    _.forEach(this.namedBuilds, function(build) {
      build.commandHandler.dispose();
    });

    _.forEach(namedBuilds, function(build, name) {
      if (_.include(self.reservedCommands, name)) { return; }
      var commandHandler = atom.commands.add(
        'atom-workspace',
        'build:' + name,
        self.build.bind(self, null, name)
      );
      self.namedBuilds[name] = _.extend(build, {
        commandHandler: commandHandler
      });
    });
  },

  showNamedBuilds: function() {
    var namedBuildsView = new NamedBuildsView(_.keys(this.namedBuilds));
    namedBuildsView.show(function(name) {
      var workspaceElement = atom.views.getView(atom.workspace);
      atom.commands.dispatch(workspaceElement, 'build:' + name);
      namedBuildsView.hide();
    });
  },

  buildCommand: function(name) {
    var path = atom.project.getPaths()[0]; // Only support root path for now.
    var cmd = {
      env: {},
      args: [],
    ...
    if (name) {
      cmd = _.defaults(this.namedBuilds[name], cmd);
    } else {
  ...
  startNewBuild: function(name) {
    this.cmd = {};
    try {
      this.cmd = this.buildCommand(name);
    } catch (error) {
  ...
  build: function(event, name) {
    clearTimeout(this.finishedTimer);

    var self = this;
    this.doSaveConfirm(this.unsavedTextEditors(), function() {
      if (self.child) {
        self.abort(self.startNewBuild.bind(self, name));
      } else {
        self.startNewBuild(name);
      }
    });
  },
